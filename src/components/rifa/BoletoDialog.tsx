...
/** Mensaje oficial de bienvenida por WhatsApp. `__________` se reemplaza por el nombre. */
function mensajeWhatsApp(nombre: string) {
  return `🎉 ¡Mucha suerte, ${nombre} ! 🍀

Gracias por escoger tu número en Sorteos El Analista. 🙌🔥

Ya tu número queda registrado ✅
Te adjunto la imagen como comprobante de tu participación.

🍀 ¡Muchísima suerte y que ese número sea el ganador! 🏆🎉
...
export function BoletoDialog({ numero, fila, onClose, onChanged, usuario }: Props) {
  ...
  const whatsapp = async () => {
    ...
    if (!tel) {
      toast.error("Registra el teléfono del participante para enviar por WhatsApp");
      return;
    }
    const nombreFinal = (nombre || boleto?.nombre || "").trim();
    if (!nombreFinal) {
      toast.error("Escribe el nombre del participante");
      return;
    }
          ...
          <div className="grid gap-6 md:grid-cols-[1fr_300px]">
            ...
            <div className="space-y-2">
              ...
              <div className="grid gap-2">
                ...
                <Button
                  onClick={whatsapp}
                  disabled={!preview || !telefono.trim()}
                  title={
                    !preview
                ...
                >
                  <IconoWhatsApp className="size-4" /> ENVIAR POR WHATSAPP
                </Button>
              </div>
            </div>
          </div>
